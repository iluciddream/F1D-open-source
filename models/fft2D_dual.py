...
class FourierMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_freq = torch.fft.fft2(x).real  # 二维FFT取实部
        return self.proj(x_freq)

class DualPathMixer(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.fft_path = FourierMixer(d_model)
        self.conv_path = nn.Conv1d(d_model, d_model, kernel_size=3, padding=1)
        self.alpha = nn.Parameter(torch.zeros(d_model))  # 每个通道一个权重，初始 sigmoid=0.5

    def forward(self, x):
        # x: [B, L, D]
        x_freq = self.fft_path(x)  # [B, L, D]
        x_time = self.conv_path(x.transpose(1, 2)).transpose(1, 2)  # [B, L, D]

        weight = torch.sigmoid(self.alpha).view(1, 1, -1)  # [1, 1, D]，广播到 batch 和序列维
        fusion_output = weight * x_freq + (1 - weight) * x_time  # 融合

        return fusion_output + x  # 残差连接
...
class FNetEncoderLayer(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        # 傅里叶混合 + 残差
        x = x + self.fourier_mixer(self.norm1(x))
        # 前馈网络 + 残差
        x = x + self.ffn(self.norm2(x))
        return x, None  # 无注意力权重输出
